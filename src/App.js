      ...
      {slides.map((slide, index) => {
        ...
        return (
            <div 
              ...
              className={`absolute inset-0 w-full h-full transition-opacity duration-[1200ms] ease-in-out ${isActive ? "opacity-100 z-10 cursor-pointer" : "opacity-0 z-0 pointer-events-none"}`}
            >
              {slide.type === "video" ? (
       ...
       <div className="bg-neutral-900 p-6 rounded-xl border border-neutral-800 space-y-6">
          <div className="flex justify-between items-center border-b border-neutral-800 pb-4">
             ...
             <div className="flex items-center gap-4">
                 ...
                 >
                    {formData.showSlogan ? <ToggleRight size={32} className="text-green-500" /> : <ToggleLeft size={32} className="text-neutral-500" />}
                 </button>
             </div>
          </div>
...
// ... (MainView, AppContent, App remain unchanged) ...
...
        if (pathParts[0] === 'works') setState({ view: 'works', showAbout: false });
        else if (pathParts[0] === 'about') setState({ view: 'home', showAbout: true });
